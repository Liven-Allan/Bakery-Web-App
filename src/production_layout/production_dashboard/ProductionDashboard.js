...
// Helper function to format raw materials vertically
    ...
    const fetchData = async () => {
      try {
        ...
        const formattedProductionDetails = productionData.map(item => {
          const { productName, quantityProduced, productionDate, unit_price } = item;

          return {
            date: formatDateTime(productionDate),  // Ensure productionDate is formatted correctly
            productName,
            quantityProduced,
            unitPrice: unit_price,  // Unit price from production data
            totalCost: (quantityProduced * parseFloat(unit_price)).toFixed(2),  // Calculate total cost
          };
